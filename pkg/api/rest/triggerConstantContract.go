...
import (
	...
	"github.com/kattana-io/tron-objects-api/pkg/utils"
	"strconv"
...
const (
	DummyCaller = "410000000000000000000000000000000000000000"
...
func (a *API) GetTokenDecimals(token string) (int32, error) {
	...
	result, err := strconv.ParseInt(utils.TrimZeroes(data.ConstantResult[0]), 16, 16)
	if err != nil {
		return defaultDecimals, err
	}
...
func (a *API) GetToken0(pair string) (string, error) {
	...
	return utils.TrimZeroes(data.ConstantResult[0]), nil
}
...
func (a *API) GetFactoryPair(factory, factoryOwner *types.Address, parameter string) (*types.Address, error) {
	...
	if err != nil {
		return types.NewFromHex("0x0"), err
	}
	...
	return types.NewFromHex(utils.TrimZeroes(data.ConstantResult[0])), nil
}
