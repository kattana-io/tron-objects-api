...
func NewPair(impl *rest.API, address types.Address) *Pair {
	return &Pair{
		api:     impl,
		address: address,
	}
...
func (s *Pair) GetTokenAddress() (*types.Address, error) {
	res, err := s.api.GetPairToken(s.address.ToHex())
	if err != nil {
		return types.NewEmptyAddress(), err
	}
	if res == "" {
		return types.NewEmptyAddress(), errors.New("returned nil address")
	}
	return types.NewFromHex(res), nil
}
func (s *Pair) GetToken0Address() (*types.Address, error) {
...
func (s *Pair) GetToken1Address() (*types.Address, error) {
